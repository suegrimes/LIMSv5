...
class Category < ApplicationRecord
  ...
  def self.populate_dropdowns(cgroups=nil)
    ...
    if !cgroups.nil?
      self.includes(:category_values).order("categories.category, category_values.c_position").where(*condition_array).references(:category_values).all
    else
      self.includes(:category_values).order("categories.category, category_values.c_position").references(:category_values).all
    end
  end
