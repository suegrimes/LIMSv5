class SampleCharacteristicsController < ApplicationController
  ...
  before_action :dropdowns, :only => [:new_sample, :edit]
  before_action :sample_dropdowns, :only => [:new_sample, :edit, :add_new_sample]
...
logger.debug "condition_array: #{condition_array.inspect}"
  ...
  def edit
    @sample_characteristic = SampleCharacteristic.includes(:samples).where('samples.source_sample_id IS NULL').references(:samples).find(params[:id])
  end
